...
function initHttpRequestExecuteFunc(){
    OfficeExtension.setCustomHttpRequestExecuteFunc(sendHttpRequest);
}
...
function initDefaultRequestUrlAndHeaders(newFile){
    ...
    return getAccessToken(clientId, refreshToken)
        .then(function(accessToken){
            ...
            if (newFile){
                return createEmptyExcelFile(accessToken);
            }
            else{
                return "AgaveTest.xlsx";
            }
        })
        .then(function(filename){
            var url = "https://graph.microsoft.com/v1.0/me/drive/root:/" + filename + ":/workbook";
            return createSessionAndBuildUrlAndHeaders(url, accessTokenValue);
        })
...
function createEmptyExcelFile(accessToken){
    var date = new Date();
    var filename = "ShaoZhu" + date.getFullYear() + "-" + date.getMonth() + "-" + date.getDay() + "-" + date.getHours() + "-" + date.getMinutes() + "-" + date.getSeconds() + ".xlsx";
    ...
    var bitmap = new Buffer(blankXlsxFileBase64Content, 'base64');
    return sendHttpRequest({
        url: url,
        method: "PUT",
        body: bitmap,
